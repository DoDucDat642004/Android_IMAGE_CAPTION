...
class OnlineModel(private val listener: IModel) {
    ...
    internal fun uploadImageToServer(imageBytes: ByteArray) {
        ...
        listener.coroutineScope.launch(Dispatchers.IO) {
            try {
                // Tạo request body dạng multipart/form-data chứa ảnh và ngôn ngữ
                val requestBody = MultipartBody.Builder()
                    .setType(MultipartBody.FORM) // Xác định kiểu dữ liệu multipart
                    .addFormDataPart(
                        "file", "image.jpg",
                        imageBytes.toRequestBody("image/jpeg".toMediaTypeOrNull()) // Gửi ảnh image/jpeg
                    )
                ...
                client.newCall(request).execute().use { response ->
                    // Nếu phản hồi không thành công --> ngoại lệ
                ...
                withContext(Dispatchers.Main) {
                    // Hiển thị thông báo lỗi theo ngôn ngữ chọn
